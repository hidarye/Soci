...
export default function RootLayout({
  ...
  return (
    <html
      ...
      data-locale="ar"
      data-shell-ready="0"
      data-shell-sidebar-collapsed="0"
      data-boot-splash="0"
    >
      <head>
        <script
  ...
  var localeKey = 'socialflow_locale_v1';
  var storedLocale = readStorage(localeKey);
  var resolvedLocale = storedLocale === 'en' || storedLocale === 'ar' ? storedLocale : 'ar';
  root.lang = resolvedLocale;
  root.dir = resolvedLocale === 'ar' ? 'rtl' : 'ltr';
  root.setAttribute('data-locale', resolvedLocale);

  // Theme init (compatible with next-themes defaults)
  var storedTheme = readStorage('theme');
  var preferredDark = match('(prefers-color-scheme: dark)');
  ...
  var isCompactViewport = match('(max-width: 767px)');
  var isTabletViewport = match('(min-width: 768px) and (max-width: 1023px)');
  var rawCollapsed = readStorage(shellSidebarKey);
  var hasStoredCollapsed = rawCollapsed === '1' || rawCollapsed === '0';
  var sidebarCollapsed = hasStoredCollapsed ? rawCollapsed === '1' : isTabletViewport;
